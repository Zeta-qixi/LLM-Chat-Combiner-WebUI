...
const ChatWindow: React.FC<ChatWindowProps> = ({ messages, onSend, isProcessing, ready }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/20">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-2xl p-3 text-xs leading-relaxed ${m.role === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-slate-800 text-slate-200 rounded-bl-none'}`}>
              {m.content}
            </div>
          </div>
        ))}
        {isProcessing && (
          <div className="flex justify-start">
            <div className="bg-slate-800/50 rounded-2xl px-4 py-2 flex items-center gap-2">
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-900/50">
        {!ready && (
          <div className="mb-2 text-[9px] text-red-400 flex items-center gap-2 font-bold uppercase">
        ...
        <form onSubmit={handleSubmit} className="relative">
          <textarea
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none h-16 disabled:opacity-30"
          />
          <button 
            type="submit" disabled={!ready || !input.trim() || isProcessing}
            className="absolute bottom-3 right-3 w-8 h-8 bg-blue-600 rounded-lg text-white disabled:opacity-20 hover:bg-blue-500 transition-colors"
          >
